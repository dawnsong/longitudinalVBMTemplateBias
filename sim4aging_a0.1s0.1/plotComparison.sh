...
matlabj <<eom
m=dlmread('tAges-mean.log', '',1) %ignore 1st line, the header
s=dlmread('tAges-sigma.log', '', 1)
...
x1=1:size(m,1);
x2=x1+0.; x3=x1+0.;
he1=errorbar(x2, m(:,1), s(:,1));
he2=errorbar(x3, m(:,2), s(:,2));
he3=errorbar(x1, m(:,3), s(:,3));
%ha.LineWidth=2;
...
print -dpng cmpAgeSubjSpecificTemplate.png
eom
